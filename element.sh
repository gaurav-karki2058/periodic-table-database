...
then
  ...
  QUERY_BY_ATOMIC_NUMBER=$($PSQL"SELECT * FROM PROPERTIES WHERE ATOMIC_NUMBER=$ATOMIC_NUMBER")
  
  IFS='|' read -r ATOMIC_NUM ELEMENT_TYPE ATOMIC_WEIGHT MELTING_POINT BOILING_POINT <<< "$QUERY_BY_ATOMIC_NUMBER"
